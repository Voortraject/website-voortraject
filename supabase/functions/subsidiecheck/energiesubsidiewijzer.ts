// ⚠️ KOPIE — bron van waarheid is src/lib/subsidies/energiesubsidiewijzer.ts.
// Enige verschil met het origineel: de import hieronder eindigt op ".ts" (Deno
// eist expliciete extensies). Houd deze kopie bij wijzigingen in de parser
// gelijk aan de bron. De parser wordt door 25 tests gedekt (src/test/).
//
...
// de tests (jsdom), in de browser én in deze Supabase edge function (Deno).

import type { Bewonertype, Maatregel, SubsidieNiveau, SubsidieRegeling, SubsidieType } from "./types.ts";
import { ALLE_MAATREGELEN } from "./types.ts";
